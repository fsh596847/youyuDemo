...
public class LFRecyclerView extends RecyclerView {
  ...
  class LFAdapterDataObserver extends AdapterDataObserver {
    ...
    public void onItemRangeMoved(int fromPosition, int toPosition, int itemCount) {
      ...
      lfAdapter.notifyItemMoved(fromPosition + lfAdapter.getheaderViewCount(),
          toPosition + lfAdapter.getheaderViewCount());
    }
  }
  ...
  public void onScrollChange(View view, int i, int i1) {
    ...
    if (isAutoLoadMore
        && (layoutManager.findLastVisibleItemPosition() == lfAdapter.getItemCount()
        - 1)
        && currentLastNum != layoutManager.findLastVisibleItemPosition()
        && num > 0
        && adapter.getItemCount() > num) {
      currentLastNum = layoutManager.findLastVisibleItemPosition();
      startLoadMore();
    }
