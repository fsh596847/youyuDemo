...
public class BSBillAdapter extends RecyclerView.Adapter<BSBillAdapter.TViewHolder> {
  ...
  private static final String TAG = BSBillAdapter.class.getSimpleName();
  ...
  public void onBindViewHolder(final BSBillAdapter.TViewHolder holder, final int position) {
    ...
    holder.currentPeriod_period.setText(
        billItem.getCurrentPeriod() + "/" + billItem.getRowsCount() + "期");
    ...
    if ("2".equals(billItem.getStatus())) {
      ...
      holder.currentInterest.setText("还款金额:"
          + billItem.getCurrentPrincipalInterest()
          + "(含利息:"
          + billItem.getCurrentInterest()
          + ")");
    } else {
      holder.totalPay.setText(Html.fromHtml(String.format(context.getString(R.string.totalPay),
          billItem.getCurrentPrincipalInterest())));
    ...
    holder.itemView.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        LogUtils.d(TAG, "------itemView--onClick---------" + position);
