...
public class ChangeBankActivity extends  BaseActivity implements View.OnClickListener {
    ...
    private static final String TAG = "ChangeBankActivity";

    private TopNavigationView2 topbar;
    private Intent intent;
    private EditText name;//姓名
    private EditText id_num;//身份证
    private EditText bank_num;//银行卡号
    private EditText phone;//手机号
    private TextView mYzmBtn;//验证码
    private EditText validateCode;//短信验证码
    ...
    public void init() {
        ...
        if(TextUtils.isEmpty(sharePrefer.getCustName()) || TextUtils.isEmpty(sharePrefer.getIdCardNum())){
    ...
    private void changeBankCard(){
        ...
        //M100141 新增银行卡
        requestParams.clear();
        requestParams.put("transCode", "M100141");//接口标识
...
//        showLoading("获取验证码中,请稍后...");
    ...
    private void m000139() {
        //协议查询
        requestParams.clear();
        requestParams.put("transCode", "M000139");//接口标识
        requestParams.put("channelNo", Constants.CHANNEL_NO);//渠道标识
        requestParams.put("clientToken", sharePrefer.getToken());//登录后token
        requestParams.put("legalPerNum", "00001");
