...
public class PaymentConfirmActivity extends  BaseActivity  {
    ...
    String  loanId , totalAmount, bankMsg;

    String currentData;
    Intent intent;
    int currentStatus; //逾期还款 1 ，全额还款 2，账单还款 3
...
//            payBill();//全额还款接口
    ...
    private void goResultActivity(String data) {
        ...
        try {
            ...
            if("000000".equals(jsonObject.getString("returnCode"))){
                ...
                mainMessage.setUrl(Constants.BASE_URL + sharePrefer.getPaymentStatus()+
                        "?result=success");
    ...
    private void payBill(){
        requestParams.clear();
        requestParams.put("transCode", "M100800");//接口标识
        requestParams.put("channelNo", Constants.CHANNEL_NO);//渠道标识
        requestParams.put("clientToken", sharePrefer.getToken());//登录后token
...
//                arrayList.append("{" + "repayingDetailId:" + "\"" + list.get(a) + "\"" + "}");
        ...
        requestParams.clear();
        requestParams.put("transCode", "M100724");//M100724 手机逾期还款
        requestParams.put("channelNo", Constants.CHANNEL_NO);//渠道标识
        requestParams.put("clientToken", sharePrefer.getToken());//登录后token
        requestParams.put("legalPerNum", "00001");
        requestParams.put("loanId", loanId);
                ...
                try {
                    ...
                    if("000000".equals(jsonObject.getString("returnCode"))){
                        ...
                        mainMessage.setUrl(Constants.BASE_URL + sharePrefer.getPaymentStatus()+
                                "?result=success");
//                        intent.putExtra("title","还款成功");
//                        intent.putExtra("url",Constants.BASE_URL + sharePrefer.getPaymentStatus()+
//                                "?result=success");
                    }else {
                        mainMessage .setTitle("还款失败");
                        mainMessage.setUrl(Constants.BASE_URL + sharePrefer.getPaymentStatus()+
