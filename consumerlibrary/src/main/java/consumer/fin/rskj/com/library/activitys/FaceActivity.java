...
import consumer.fin.rskj.com.library.utils.FaceMask;
import consumer.fin.rskj.com.library.utils.ICamera;
import consumer.fin.rskj.com.library.utils.IDetection;
import consumer.fin.rskj.com.library.utils.IMediaPlayer;
import consumer.fin.rskj.com.library.utils.LogUtils;
import consumer.fin.rskj.com.library.utils.Screen;
import consumer.fin.rskj.com.library.utils.SensorUtil;
import consumer.fin.rskj.com.library.views.CircleProgressBar;
...
public class FaceActivity extends BaseActivity
  ...
  private Runnable mTimeoutRunnable = new Runnable() {
    @Override public void run() {
      // 倒计时开始
      initDetecteSession();
      if (mIDetection.mDetectionSteps != null) changeType(mIDetection.mDetectionSteps.get(0), 10);
    }
  };
   ...
   */
  @Override public DetectionType onDetectionSuccess(final DetectionFrame validFrame) {
    mIMediaPlayer.reset();
    mCurStep++;
      ...
      @Override public void run() {
        ...
        for (String key : idDataStruct.images.keySet()) {
          ...
          if (key.equals("image_best")) {
            ...
            LogUtils.d("debug", "活体检测: imageBestStr length--->" + imageBestStr.length());

            checkFace(sharePrefer.getPhone(), imageBestStr, delta);
  ...
  private void checkFace(final String phone, String faceimage, String delta) {
    ...
    requestParams.put("transCode", Constants.TRANS_CODE_FACE_CHECK);//接口标识
    ...
    sendPostRequest(requestParams, new ResultCallBack() {
      @Override public void onSuccess(String data) {
        ...
        try {
          ...
          if ("00".equals(code)) {
            LogUtils.d("debug", "活体检测成功");
            handleResult(R.string.verify_success);
          } else if ("02".equals(code)) {
            LogUtils.d("debug", "活体检测失败" + code);
      ...
      @Override public void onError(String retrunCode, String errorMsg) {
        LogUtils.d(TAG, "活体检测: errorMsg--->" + errorMsg);
        handleResult(R.string.verify_error);
      }
   ...
   */
  @Override public void onDetectionFailed(final DetectionFailedType type) {
   ...
   */
  @Override public void onFrameDetected(long timeout, DetectionFrame detectionFrame) {
  ...
  @Override public void onClick(View view) {
    if (view.getId() == R.id.detection_step_cancle_btn) {
