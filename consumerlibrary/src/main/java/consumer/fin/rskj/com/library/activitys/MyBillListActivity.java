...
public class MyBillListActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        try {
            ...
            isDelay = "2".equals(jsonObject.getString("states")) ? true : false;
...
//        headerView = layoutInflater.inflate(R.layout.activity_billheader,null);
        ...
        pAdapter.setOnItemClickListener(new BillAdapter.OnItemClickListener() {
            @Override
            public void OnItemClick(View view, BillAdapter.TViewHolder holder, int position) {
...
//        recyclerView.setScrollChangeListener(this);
    ...
    private void getBill(){
        //M100611 本月账单接口
        requestParams.clear();
        requestParams.put("transCode", "M100611");//接口标识
        requestParams.put("channelNo", Constants.CHANNEL_NO);//渠道标识
        requestParams.put("clientToken", sharePrefer.getToken());//登录后token
        requestParams.put("legalPerNum", "00001");
                ...
                try {
                    ...
                    for (int a = 0 ; a < jsonArray.length(); a++){
                        ...
                        try{
                            ...
                            plItem.setCurrentEndDate("---");//计划还款日
                        }

                        try{
                            plItem.setCurrentPeriod(object.getString("currentPeriod"));//当期期数
                        }catch (Exception e){
                            plItem.setCurrentPeriod("0");//当期期数
                        }

//                        total = total.add(new BigDecimal(object.getString("totalPay")));
...
//                        Rows row = new Rows();
//                        row.setLoanId(object.getString("loanId"));
//                        row.setRepayAmt(object.getString("totalPay"));
//                        rows.add(row);
                    }
