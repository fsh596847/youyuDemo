...
import consumer.fin.rskj.com.library.utils.DialogUtil;
import consumer.fin.rskj.com.library.utils.IDCardCamera;
import consumer.fin.rskj.com.library.utils.RotaterUtil;
import consumer.fin.rskj.com.library.utils.Util;
import consumer.fin.rskj.com.library.views.IDCardIndicator;
import consumer.fin.rskj.com.library.views.IDCardNewIndicator;
...
public class IDCardScanActivity extends Activity
  ...
  private void init() {
    ...
    mICamera = new IDCardCamera(mIsVertical);
  ...
  private void launchDebugMode() {
    ...
    if (continuousClickCount == 0 || (continuousClickCount > 0
        && currentMillis - lastClickMillis < 200)) {
  ...
  private void setDebugRectanglePosition() {
    ...
    ViewGroup.MarginLayoutParams params =
        (ViewGroup.MarginLayoutParams) debugRectangle.getLayoutParams();
  ...
  private void initData() {
    idCardQualityAssessment = new IDCardQualityAssessment.Builder().setIsIgnoreShadow(true)
        .setIsIgnoreHighlight(true)
        .build();
  ...
  private class DecodeThread extends Thread {
    ...
    @Override public void run() {
      ...
      try {
        while ((imgData = mFrameDataQueue.take()) != null) {
          ...
          if (!isEven01(roi.left)) roi.left = roi.left + 1;
          if (!isEven01(roi.top)) roi.top = roi.top + 1;
          if (!isEven01(roi.right)) roi.right = roi.right - 1;
          if (!isEven01(roi.bottom)) roi.bottom = roi.bottom - 1;

          final IDCardQualityResult result =
              idCardQualityAssessment.getQuality(imgData, imageWidth, imageHeight, mSide, roi);
            ...
            @Override public void run() {
              if (isDebugMode) {
                if (result != null && result.attr != null) {
                  String debugResult =
                      "clear: "
                          + new BigDecimal(result.attr.lowQuality).setScale(3,
                          BigDecimal.ROUND_HALF_UP).doubleValue()
                          + "\n"
                          + "in_bound: "
                          + new BigDecimal(result.attr.inBound).setScale(3,
                          BigDecimal.ROUND_HALF_UP).doubleValue()
                          + "\n"
                          + "is_idcard: "
                          + new BigDecimal(result.attr.isIdcard).setScale(3,
                          BigDecimal.ROUND_HALF_UP).doubleValue()
                          + "\n"
                          + "flare: "
                          + result.attr.specularHightlightCount
                          + "\n"
                          + "shadow: "
                          + result.attr.shadowCount
                          + "\n"
                          + "millis: "
                          + perFrameMillis;
          ...
          if (result != null) {
            ...
            if (result.isValid()) {
              vibrator.vibrate(new long[] { 0, 50, 50, 100, 50 },
                  -1);           //重复两次上面的pattern 如果只想震动一次，index设为-1
              mHasSuccess = true;
              ...
              runOnUiThread(new Runnable() {
                @Override public void run() {
                  List<IDCardQualityResult.IDCardFailedType> failTypes =
                      (result == null) ? null : result.fails;
                  if (failTypes != null) {
                  ...
                  if (mCount != 0) fps.setText((1000 * mCount / mTimSum) + " FPS");
                }
              });
            }
          }
    ...
    private void handleSuccess(IDCardQualityResult result) {
      Log.e(IDCardScanActivity.class.getSimpleName(),
          "handleSuccess1(ms): " + System.currentTimeMillis());
      long start = System.currentTimeMillis();
      ...
      Log.e(IDCardScanActivity.class.getSimpleName(),
          "handleSuccess2(ms): " + System.currentTimeMillis());
      finish();
    }
  }
  /**
