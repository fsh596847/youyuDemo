 ...
 * 绑定身份证页面
 */
public class FaceIDCardInfoUploadActivity extends BaseActivity implements
        OnClickListener {
    ...
    private String uuid = "";//face++身份证识别SDK所需的uuid
    ...
    //name:姓名 idcardNum：身份证号 address：户籍地址 birthday：初始年月 people:民族
    // sex：性别 issue_authority：签发机关 validity：身份证有效期 bankCardNum：银行卡号
    private String name = "", idcardNum = "", address = "", idcardType = "", birthday = "", people = "",
            sex = "", issue_authority = "", validity = "", custCardFront = "", custCardVerso = "",
            bankCardNum = "";
    ...
    private void netWorkWarring() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                IDCardQualityLicenseManager idCardLicenseManager = new IDCardQualityLicenseManager(FaceIDCardInfoUploadActivity.this);
                manager.registerLicenseManager(idCardLicenseManager);
                manager.takeLicenseFromNetwork(uuid);
                String contextStr = manager.getContext(uuid);
                LogUtils.d("debug", "contextStr====" + contextStr);
                LogUtils.d("debug", "idCardLicenseManager.checkCachedLicense()===" + idCardLicenseManager.checkCachedLicense());
...
//            startActivity(intent);
    ...
    // 身份信息更新接口
    private void upLoadIdCardInfo(/*final String modifyName, final String idnumber, String birthday, String people, String sex, String address, String validity*/) {
       //name, idcardNum, birthday, people, sex, address, validity

        showLoading("信息上传中，请稍后...");
        mIdcardInfoSubmitBtn.setClickable(false);
        mIdcardInfoSubmitBtn.setBackgroundResource(R.mipmap.m_icon_common_button_selected_bg);
        requestParams.clear();
        requestParams.put("transCode", "M100122");//接口标识
