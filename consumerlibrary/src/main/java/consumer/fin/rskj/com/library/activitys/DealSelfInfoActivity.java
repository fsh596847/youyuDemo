...
public class DealSelfInfoActivity extends BaseActivity implements  OnClickListener {
    private static final String TAG = "DealSelfInfoActivity";

    private View mStatusView;
    private Button mBackBtn, mYzmBtn, mSubmitBtn;
    private TextView mTitleView, /*mOpenNameTv, mOpenIdCardTv,*/ mLinesWeituoshuTv, mLinesHaunkuanTv, mLinesJiekuanTv, mLinesSelfInfoZhengxinTv;

    EditText mOpenNameTv,mOpenIdCardTv;
    private ImageView open_card_warn_btn;
    private Dialog mBanksPopWindow;
    private List<String> itemList = new ArrayList<String>();

    private EditText mMoblieEt, mYzmEt, mOpenBankCardIdEt;
    private TextView mAllLinesTv;
    private CheckBox mLinesCheckbox;
    private long surplusTime = 0;
    private long allsurplusTime = 120000;
    private String openName, openIdcard, openBankCardId, openMobile, openYzm;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LogUtils.d("DealSelfInfoActivity","数组 = " + SysUtil.stepMap);
    }
...
//        }
        ...
        mLinesSelfInfoZhengxinTv = (TextView) findViewById(R.id.lines_person_info_zhengxin_tv);//个人信息及征信查询授权书
        ...
        SpannableStringBuilder spannable = new SpannableStringBuilder(/*getResources().getString(R.string.lines_text)*/"我已阅读并同意 " + temp);

        mAllLinesTv.setMovementMethod(LinkMovementMethod.getInstance());

        if(contractNameList != null && contractNameList.length > 0){
            spannable.setSpan(new AccountLinesTextClick(), 8, 8 + contractNameList[0].length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            spannable.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.color_eb8311)), 8, 8 + contractNameList[0].length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

        if(contractNameList != null && contractNameList.length > 1) {
            spannable.setSpan(new PersonLinesTextClick(), 8 + contractNameList[0].length(), 8 + contractNameList[0].length() + contractNameList[1].length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            spannable.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.color_eb8311)), 8 + contractNameList[0].length() , 9 + contractNameList[0].length() + contractNameList[1].length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

        if(contractNameList != null && contractNameList.length  >2 ) {
            spannable.setSpan(new ZhenXinLinesTextClick(), 8 + contractNameList[0].length() + contractNameList[1].length(), spannable.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            spannable.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.color_eb8311)), 8 + contractNameList[0].length() + contractNameList[1].length(), spannable.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

        mAllLinesTv.setText(spannable);

        mAllLinesTv.setHighlightColor(getResources().getColor(android.R.color.transparent));//方法重新设置文字背景为透明色
    ...
    private class AccountLinesTextClick extends ClickableSpan {
        ...
        public void onClick(View widget) {
            ...
            intent.putExtra("url",Constants.BASE_URL +
                    contractUrlList[0]+"?name= "  + sharePrefer.getCustName() + "" +"& idcard=" + sharePrefer.getIdCardNum());
    ...
    private class PersonLinesTextClick extends ClickableSpan {
        ...
        public void onClick(View widget) {
            ...
            intent.putExtra("url",Constants.BASE_URL + contractUrlList[1]+"?name= "  + sharePrefer.getCustName() + "" +"& idcard=" + sharePrefer.getIdCardNum());
            startActivity(intent);
        }
    ...
    private class ZhenXinLinesTextClick extends ClickableSpan {
        ...
        public void onClick(View widget) {
            ...
            intent.putExtra("url",Constants.BASE_URL + contractUrlList[2]+"?name=  "  + sharePrefer.getCustName() + "" +"& idcard=" + sharePrefer.getIdCardNum());
            startActivity(intent);
        }
    ...
    private TextWatcher textWacher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable text) {
            ...
            //根据各项信息判断提交按钮是否可用
            if (openName != null && !openName.equals("") && openIdcard != null && !openIdcard.equals("")
                    && openBankCardId != null && !openBankCardId.equals("") && openBankCardId.length() > 10 && openBankCardId.length() <= 25
                    && openMobile != null && !openMobile.equals("")
                    && openYzm != null && !openYzm.equals("")) {
                //可用状态
...
//            DealSelfInfoActivity.this.finish();
        ...
        sendPostRequest(requestParams, new ResultCallBack() {
            ...
            public void onSuccess(String data) {
                ...
                try {
                    ...
                    if ("2".equals(type)) {
                        //账户委托扣款授权书
                        WebViewActivity.openWebViewActivity(DealSelfInfoActivity.this, "账户委托扣款授权书", Constants.BASE_URL + contractUrlList[0]);
                    } else if ("3".equals(type)) {
                        //个人信息使用授权书
                        WebViewActivity.openWebViewActivity(DealSelfInfoActivity.this, "个人信息使用授权书", Constants.BASE_URL + contractUrlList[1]);
                    } else if ("4".equals(type)) {
                        //征信查询授权书
                        String personalInformationURL = Constants.BASE_URL + contractUrlList[2] + "?username=" + openName + "&idcard=" + openIdcard;
                        WebViewActivity.openWebViewActivity(DealSelfInfoActivity.this, "征信查询授权书", personalInformationURL);
                    }
                } catch (JSONException e) {
    ...
    // 是否是清远农商银行卡验证接口
    private void isQingyuanBankCard(String bankCardid, String mobile,
...
//        });
    ...
    // 银行卡验证4接口
    private void bindBankCard(final String bankCardid, String mobile,
        ...
        showLoading("信息上传中，请稍后...");
        requestParams.clear();
        requestParams.put("transCode", "M100106");//接口标识
        requestParams.put("channelNo", Constants.CHANNEL_NO);//渠道标识
        requestParams.put("clientToken", sharePrefer.getToken());//登录后token
...
//                        Intent intent = new Intent(DealSelfInfoActivity.this, FaceStartActivity.class);
    ...
    private void initAllBanksDialog() {
        ...
        View contentView = inflater.inflate(
                R.layout.support_bank_layout, null);
        ...
        RecyclerView recyclerView = (RecyclerView) contentView
                .findViewById(R.id.bank_listview);
    ...
    private void M108103(){
        ...
        showLoading("请稍后...");
        requestParams.clear();
        requestParams.put("transCode", "M108104");//接口标识
        requestParams.put("channelNo", Constants.CHANNEL_NO);//渠道标识
        requestParams.put("clientToken", sharePrefer.getToken());//登录后token
