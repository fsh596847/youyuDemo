...
public class MenuListActivity2 extends HXBaseActivity {
  ...
  private void initTabHost() {
    ...
    if (getSharePrefer().iSLogin()) {
      if (getSharePrefer().isShowTab()) {
        ...
        Tab num3 =
            new Tab(R.string.tap_center, R.drawable.m_tab3selector, PersonCenterFragment.class);
  ...
  private Tab getHomeTab() {
    ...
    if ("mashanghuan".equals(getSharePrefer().getIdentity())) {
      ...
      num1 = new Tab(R.string.tap_home, R.drawable.m_tab1selector, /*XiaoJFragment.class*/
          FinacalFragment15.class);
...
/*            ToastUtils.showCenterToast( "授信申请被拒绝，请重新申请..." ,this);
      ...
      intent2.putExtra("option",
          "check");//option: add(添加银行卡)、check(直接进入银行卡验证页面)、checkThree(开启验证三流程)
