 ...
 * 验证结果页面
 */
public class XJDealResultActivity extends XJBaseActivity implements OnClickListener {
	...
	public void onClick(View view) {
		...
		case R.id.left_btn:
			...
			{
				//失败返回银行卡验证页面
				XJDealResultActivity.this.finish();
			}else
			{
				//成功返回首页
//				ActivityStackManagerUtils.getInstance().finishAllActivity();
				Intent intent = new Intent(XJDealResultActivity.this,MenuListActivity2.class);
...
//			ActivityStackManagerUtils.getInstance().finishAllActivity();
		...
		case R.id.result_failure_check_three_btn:
			//开启验三
			Intent checkIntent = new Intent(XJDealResultActivity.this,XJFaceIDCardInfoUploadActivity.class);
