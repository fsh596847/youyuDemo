...
//				}
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			...
			int hPerceived = h + (mCurrentAdapterIndex%mSideBuffer - mCurrentBufferIndex)
                    * getWidth();

            if(mLastIndex%mSideBuffer==mSideBuffer-1 && mCurrentAdapterIndex>mLastIndex) {
                oldh=0;
                hPerceived = 0;
            }
            if(mLastIndex%mSideBuffer==0&&mCurrentAdapterIndex<mLastIndex)
                hPerceived=h+(mSideBuffer-1-mCurrentBufferIndex)*getWidth();
