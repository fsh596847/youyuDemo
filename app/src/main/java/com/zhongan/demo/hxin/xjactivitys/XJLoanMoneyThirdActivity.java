 ...
 * 借款借据界面
 */
public class XJLoanMoneyThirdActivity extends XJBaseActivity implements
		OnClickListener {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case Contants.MSG_DO_LOAN_APPLY_SUCCESS:
				...
				Intent successIntent = new Intent(XJLoanMoneyThirdActivity.this,
						XJLoanMoneyResultActivity.class);
			...
			case Contants.MSG_DO_LOAN_APPLY_FAILURE:
				...
				Intent failureIntent = new Intent(XJLoanMoneyThirdActivity.this,
						XJLoanMoneyResultActivity.class);
			...
			CustomDialog.Builder builder = new CustomDialog.Builder(XJLoanMoneyThirdActivity.this);
...
//		loanRate=bundle.getString("loanRate");
	...
	public void onClick(View view) {
		...
		case R.id.left_btn:
			// 返回
			XJLoanMoneyThirdActivity.this.finish();
			break;
		case R.id.loan_money_yzm_btn:
			// 获取验证码
			getMsmCode("6",sharePrefer.getPhone());
...
//				mHandler.handleMessage(msg);
				Toast.makeText(XJLoanMoneyThirdActivity.this, "验证码不能为空,请输入!", Toast.LENGTH_SHORT).show();
				return;
			}
			//判断验证码是否合法
...
//				msg.obj = "请输入6位合法验证码!";
//				mHandler.handleMessage(msg);
				Toast.makeText(XJLoanMoneyThirdActivity.this, "请输入6位合法验证码!", Toast.LENGTH_SHORT).show();
				return;
			}
		...
		case R.id.loan_money_look_hetong_tv:
			// 查看合同
			Intent useLinesIntent=new Intent(XJLoanMoneyThirdActivity.this,XJUserLineActivity.class);
	...
	private void initPasswordPopWindow(View view) {
		...
		if (mPwdEt != null) {
            keyboard.setRecvTouchEventActivity(XJLoanMoneyThirdActivity.this);
        }
//		mPwdEt.setFocusable(false);
		mPwdEt.setOnEditTextContentListener(new JYPEditText.EditTextContentListener() {
			...
			public void onComplete(CharSequence text) {
				...
				doLoanApply(applyAmount,loanTerm,password,sharePrefer.getPhone(),yzm,productId,repayWayId);
...
//					});
	...
	private void doLoanApply(String applyAmt,String loanTerm,String pwd,String moblile,String yzm,String productId,String repayWayId) {
		...
		mSubmitBtn.setClickable(false);
		mSubmitBtn.setBackgroundResource(R.drawable.hxcommon_btn_selected_bg);
		LoggerUtil.debug("贷款业务申请 : clientToken--->"+sharePrefer.getToken()+"\napplyAmt----->"+applyAmt+"\npwd------------>"+pwd);
		RequestParams params = new RequestParams("utf-8");
