 ...
 *还款结果页面
 */
public class XJPayMoneyResultActivity extends XJBaseActivity implements OnClickListener {
...
//				ActivityStackManagerUtils.getInstance().finishAllActivity();
				Intent intent = new Intent(XJPayMoneyResultActivity.this,MenuListActivity2.class);
				startActivity(intent);
			}else
			{
			   XJPayMoneyResultActivity.this.finish();
			}
			break;
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			if (payType.equals("all") && resultFlag.equals("true")) {
				// ActivityStackManagerUtils.getInstance().finishAllActivity();
				Intent intent = new Intent(XJPayMoneyResultActivity.this,
						XJMainActivity.class);
				startActivity(intent);
			} else {
				XJPayMoneyResultActivity.this.finish();
			}
