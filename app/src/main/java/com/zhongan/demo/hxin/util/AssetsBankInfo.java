...
/**
 * Created by AnmiLin on 2016/9/10. 根据银行卡号判断是哪个银行的卡，依据是银行卡号的前6位数，称之为bin号。
...
public class AssetsBankInfo {
	...
	{ 
		LoggerUtil.debug("bankBin: " + binNum);
	    int index = 0;
	    index = binarySearch(getBinNum(context), binNum); if (index == -1)
	    {
	       return "磁条卡卡号"; 
	    } 
