 ...
 * 身份证获取照片
 * */
public class XJFaceIDCardScanActivity extends Activity implements TextureView.SurfaceTextureListener, Camera.PreviewCallback {
...
//            mDialogUtil.onDestory();//销毁后释放用
	...
	private class DecodeThread extends Thread {
			...
			try {
				while ((imgData = mFrameDataQueue.take()) != null) {
					...
					imgData = RotaterUtil.rotate(imgData, imageWidth, imageHeight,
							mICamera.getCameraAngle(XJFaceIDCardScanActivity.this));
					if (mIsVertical) {
					...
					if (result != null) {
						if (result.attr != null) {
							...
							if (isIDCard > mIsIDCard && inBound > 0)
								if (!mIsVertical)
									mNewIndicatorView.setBackColor(XJFaceIDCardScanActivity.this, 0xaa000000);
								else
									mIdCardIndicator.setBackColor(XJFaceIDCardScanActivity.this, 0xaa000000);
							else if (!mIsVertical)
								mNewIndicatorView.setBackColor(XJFaceIDCardScanActivity.this, 0x00000000);
							else
								mIdCardIndicator.setBackColor(XJFaceIDCardScanActivity.this, 0x00000000);
						}
						if (result.isValid()) {
		...
		private void handleSuccess(IDCardQualityResult result) {
			...
			Util.cancleToast(XJFaceIDCardScanActivity.this);
	...
	public static void startMe(Context context, IDCardAttr.IDCardSide side) {
		if (side == null || context == null)
			return;
		Intent intent = new Intent(context, XJFaceIDCardScanActivity.class);
