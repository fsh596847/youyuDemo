...
import com.zhongan.demo.hxin.util.IDCardRotaterUtil;
import com.zhongan.demo.hxin.util.IDCardUtil;
 ...
 * 身份证获取照片
 * */
public class HXFaceIDCardScanActivity extends Activity implements
		TextureView.SurfaceTextureListener, Camera.PreviewCallback {

	private TextureView textureView;
	private DialogUtil mDialogUtil;
	private IDCardCamera mICamera;// 照相机工具类
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.hxactivity_face_idcard_scan_layout);
		init();
		initData();
	}
...
//            mDialogUtil.onDestory();//销毁后释放用
	...
	private class DecodeThread extends Thread {
			...
			try {
				while ((imgData = mFrameDataQueue.take()) != null) {
					...
					imgData = RotaterUtil.rotate(imgData, imageWidth, imageHeight,
							mICamera.getCameraAngle(HXFaceIDCardScanActivity.this));
					if (mIsVertical) {
					...
					if (result != null) {
						if (result.attr != null) {
							...
							if (isIDCard > mIsIDCard && inBound > 0)
								if (!mIsVertical)
									mNewIndicatorView.setBackColor(HXFaceIDCardScanActivity.this, 0xaa000000);
								else
									mIdCardIndicator.setBackColor(HXFaceIDCardScanActivity.this, 0xaa000000);
							else if (!mIsVertical)
								mNewIndicatorView.setBackColor(HXFaceIDCardScanActivity.this, 0x00000000);
							else
								mIdCardIndicator.setBackColor(HXFaceIDCardScanActivity.this, 0x00000000);
						}
						if (result.isValid()) {
		...
		private void handleSuccess(IDCardQualityResult result) {
			...
			Util.cancleToast(HXFaceIDCardScanActivity.this);
	...
	public static void startMe(Context context, IDCardAttr.IDCardSide side) {
		if (side == null || context == null)
			return;
		Intent intent = new Intent(context, HXFaceIDCardScanActivity.class);
