 ...
 * 人脸识别起始页
  **/
public class XJFaceStartActivity extends XJBaseActivity implements OnClickListener{
	...
	private void netWorkWarranty() {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				Manager manager = new Manager(XJFaceStartActivity.this);
				LivenessLicenseManager licenseManager = new LivenessLicenseManager(XJFaceStartActivity.this);
	...
	public void onClick(View view) {
		...
		case R.id.left_btn:
			//返回
			XJFaceStartActivity.this.finish();
			break;
		case R.id.face_start_liveness_btn:
			//开始检测
			startActivityForResult(new Intent(this, XJFaceActivity.class), PAGE_INTO_LIVENESS);
			break;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == PAGE_INTO_LIVENESS && resultCode == RESULT_OK) {
			...
			XJFaceResultActivity.startActivity(this, result);
		}
	}
